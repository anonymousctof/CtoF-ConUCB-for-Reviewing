...
class KMeans:
    ...
    def fit_predict(self, X, gamma=None):
        ...
        for _iter in range(self.max_iter):
            ...
            new_centers = new_centers[new_centers[:, 0] > -np.inf]  # 筛选掉没有
            ...
            if centers.shape[0] == new_centers.shape[0]:
                if ((new_centers - centers).norm(dim=1) < self.tol).sum() == self.n_clusters:
                    if centers.shape[0] != self.n_clusters:
                        print("not equal centers.shape, self.n_clusters", centers.shape, self.n_clusters)
                        input()
                    else:
                        print("equal centers.shape, self.n_clusters", centers.shape, self.n_clusters)
                        input()
                    break
            centers = new_centers.detach().clone()
        print("kmeans select.shape", select.shape)
